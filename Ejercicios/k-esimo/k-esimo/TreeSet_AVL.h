    ...
    */
    ...
    struct TreeNode {
        ...
        TreeNode(T const& e, Link i = nullptr, Link d = nullptr,
                 int alt = 1, int tam = 1) : elem(e), iz(i), dr(d), altura(alt),
                 tam_i(tam) {}
   };
...
public:
    ...
    Link buscaKesimo (long long int const& k, Link nodo) const{
        if (nodo == nullptr) return nullptr;
        else if (nodo->tam_i < k) return buscaKesimo((k - nodo->tam_i), nodo->dr);
        else if (nodo->tam_i > k) return buscaKesimo(k, nodo->iz);
        else return nodo;
    }
    
    T const& kesimo(int k) const{
        Link nodo = buscaKesimo(k, raiz);
        if ( nodo != nullptr){
            return nodo->elem;
        }else{
